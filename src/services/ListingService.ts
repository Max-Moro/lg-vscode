...
export async function runListIncludedJson(params: { section?: string; mode?: "all" | "changes"; model?: string }): Promise<{ path: string; sizeBytes: number }[]> {
  const target = params.section ? `sec:${params.section}` : "sec:all";
  const data = await cliReport(target, params.model ?? "o3", params.mode);

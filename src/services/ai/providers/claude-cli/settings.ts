...
export const claudeCliSettings: ProviderSettingsModule = {
  ...
  isVisible: (state: PCEState) => state.persistent.providerId === "com.anthropic.claude.cli",

  buildViewModel: (state: PCEState): Record<string, unknown> => {
    const { model, method } = getClaudeSettings(state);

    return {
      claudeSettingsVisible: state.persistent.providerId === "com.anthropic.claude.cli",
      claudeModels: getAvailableClaudeModels().map(m => ({
        value: m.id,
        label: m.label,
        description: m.description
      })),
      selectedClaudeModel: model,
      claudeMethods: getAvailableClaudeMethods().map(m => ({
        value: m.id,
        label: m.label,
        description: m.description
      })),
      selectedClaudeMethod: method
    };
  }
};

 ...
 * Base AI Provider
 * 
 * Базовый класс для всех AI-провайдеров с общей функциональностью
 */
...
export abstract class BaseAiProvider implements IAiProvider {
  ...
  /**
   * Проверка доступности провайдера (должна быть реализована в наследниках)
   */
  abstract isAvailable(): Promise<boolean>;

  /**
   * Отправка контента (должна быть реализована в наследниках)
   */
  abstract sendContent(content: AiContent, options?: AiProviderOptions): Promise<void>;

  /**
   * Получение рекомендаций по контенту
   */
  getContentRecommendations(content: AiContent): { warnings: string[]; suggestions: string[] } {
    const warnings: string[] = [];
    const suggestions: string[] = [];
    
    // Общие проверки размера
  ...
  /**
   * Создание префикса для контента
   */
  protected createContentPrefix(content: AiContent): string {
    ...
    switch (type) {
      ...
      case 'listing':
        return `# ${icon} LG Listing: ${metadata.name}\n\n` +
               `Generated by Listing Generator from section \`${metadata.name}\`.\n` +
  ...
  /**
   * Получение опций из конфигурации VS Code
   */
  ...
  /**
   * Объединение опций (переданные перезаписывают конфигурационные)
   */
  ...
  protected async showNotification(content: AiContent, options: AiProviderOptions, success: boolean = true): Promise<void> {
    ...
    if (options.showDetailedNotifications) {
      ...
      const message = success 
        ? `✅ LG content copied to clipboard (${size})\n\n📋 Next steps:\n${instructions}`
      ...
      const message = success
        ? `📋 Content copied to clipboard (${size})`
        : `❌ Failed to send to ${this.info.name}`;
  ...
  /**
   * Попытка автоматического открытия AI панели
   */
